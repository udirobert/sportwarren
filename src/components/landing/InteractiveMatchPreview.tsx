    ...
    <div className="w-full max-w-4xl mx-auto px-2 sm:px-0">
      <Card className="relative bg-gray-950/95 border-white/10 backdrop-blur-xl overflow-hidden shadow-2xl shadow-green-500/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
          {/* Left: Pitch Preview */}
          <div ref={leftPanelRef} className="p-3 sm:p-6 border-b md:border-b-0 md:border-r border-white/[0.06] bg-gray-950">
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              <h3 className="text-[11px] sm:text-sm font-black uppercase tracking-[0.2em] text-green-400 flex items-center gap-1.5 sm:gap-2">
              ...
              <div className="flex items-center gap-0.5 bg-white/[0.06] rounded-lg p-0.5">
                <Button variant="ghost" size="sm" onClick={prevFormation} className="h-7 w-7 p-0 text-white hover:text-white hover:bg-white/10 rounded-md">
                  <ChevronLeft className="w-3.5 h-3.5" />
                </Button>
                <span className="text-sm font-mono font-bold text-white w-12 sm:w-14 text-center tabular-nums">{formation}</span>
                <Button variant="ghost" size="sm" onClick={nextFormation} className="h-7 w-7 p-0 text-white hover:text-white hover:bg-white/10 rounded-md">
            ...
            <div className="mb-3 sm:mb-4">
              ...
              <div className="grid grid-cols-4 sm:inline-flex bg-gray-800 rounded-lg p-0.5 gap-0.5 w-full sm:w-auto">
                {SQUAD_SIZES.map((size) => (
                  <button
                    key={size}
                    onClick={() => handleSquadSizeChange(size)}
                    className={`px-2 sm:px-3 py-1.5 rounded-md text-[11px] sm:text-xs font-bold text-center transition-all ${
                      squadSize === size
            ...
            <div ref={pitchRef} className="rounded-xl overflow-hidden shadow-2xl">
              <PitchCanvas
                ...
                playStyle={playStyle}
                pitchTheme={pitchTheme as 'premier-league' | 'sunday-league' | 'night-match' | 'easy-on-eyes'}
                theme="hero"
            ...
            <div className="mt-3 sm:mt-4 rounded-xl overflow-hidden border border-green-500/30 bg-gray-900">
              <div className="flex items-center gap-2 px-2 sm:px-3 py-1.5 bg-green-500/15 border-b border-green-500/20">
                <Sparkles className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-green-400" />
                <span className="text-[11px] sm:text-xs font-black uppercase tracking-[0.15em] text-green-400">AI Insight</span>
                <span className="ml-auto w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" />
              </div>
              <p className="px-2 sm:px-3 py-2 sm:py-2.5 text-sm text-white font-medium leading-relaxed">{aiInsight}</p>
            </div>
          </div>
          ...
          <div className="p-3 md:p-6 flex flex-col justify-between bg-gray-950">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {PITCH_THEMES.map((pt) => (
                      <button
                        ...
                        className={`relative px-3 py-2.5 sm:px-3 sm:py-2.5 rounded-xl border text-left transition-all overflow-hidden min-h-[56px] sm:min-h-0 ${
                          pitchTheme === pt.value 
                            ? 'border-blue-400/40 ring-1 ring-blue-400/30 shadow-lg shadow-blue-500/10' 
                            : 'bg-white/[0.06] border-white/10 hover:border-white/25 hover:bg-white/[0.08]'
                        }`}
                      >
                        {pitchTheme === pt.value && (
            ...
            <div className="mt-4 sm:mt-6 md:mt-8 space-y-2 sm:space-y-3">
              <div className="flex flex-col sm:flex-row gap-2">
                <Button 
                  className="w-full sm:flex-1 bg-gradient-to-r from-green-500 to-emerald-600 border-0 font-bold text-xs sm:text-sm py-2.5 sm:py-3"
                  onClick={handleShare}
                >
                  ...
                  <div key={i} className="rounded-xl border border-white/10 bg-white/5 p-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <label htmlFor={`editor-name-${i}`} className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1">Name</label>
                        <input
                          id={`editor-name-${i}`}
