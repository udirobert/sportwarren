...
const FORMATION_OPTIONS = [
  {
    id: "1-2-1",
    label: "5s 1-2-1",
    description: "One stays, two support, one finishes",
  },
  {
    id: "1-1-2",
    label: "5s 1-1-2",
    description: "Aggressive two-forward small-sided shape",
  },
  {
    id: "1-3-1",
    label: "6s 1-3-1",
    description: "Compact base with one central outlet",
  },
  {
    id: "1-2-2",
    label: "6s 1-2-2",
    description: "Two banks with quick counter lanes",
  },
  {
    id: "1-3-2",
    label: "7s 1-3-2",
    description: "Three-player base with two advanced runners",
  },
  {
    id: "2-3-1",
    label: "7s 2-3-1",
    description: "Wide midfield support behind a focal point",
  },
  {
    id: "4-4-2",
    label: "Classic 4-4-2",
    description: "Balanced and structured",
  },
  {
    id: "4-3-3",
    label: "Attacking 4-3-3",
    description: "Wide play and high pressure",
  },
  { id: "5-3-2", label: "Solid 5-3-2", description: "Counter-attacking depth" },
  {
    id: "4-2-3-1",
    label: "Modern 4-2-3-1",
    description: "Technical and fluid",
  },
] as const;
...
export function OnboardingFlow({
  ...
  const [personalizationStep, setPersonalizationStep] = useState<
    "identity" | "formation" | "brand"
  >("identity");
  const personalizationProgress = useMemo(() => {
    const map: Record<typeof personalizationStep, number> = {
      identity: 33,
      formation: 66,
      brand: 90,
    };
  ...
  const [hasCompletedTour, setHasCompletedTour] = useState(() => {
    ...
    return (
      localStorage.getItem(ONBOARDING_STORAGE_KEYS.TOUR_COMPLETED) === "true"
    );
  });
  ...
  const [primaryColor, setPrimaryColor] = useState<string>(
    preferences.squadBranding?.primaryColor || "#10b981",
  );
  ...
  const [pendingClaimPosition, setPendingClaimPosition] =
    useState<PlayerPosition | null>(null);
  const [pendingClaimToken, setPendingClaimToken] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    linkShareClaim.mutate(
      { claimToken: pendingClaimToken },
      {
        ...
        onError: (error) => {
          setProfileError(
            error.message || "Could not save your claimed player card.",
          );
        },
      },
    );
  ...
  useEffect(() => {
    localStorage.setItem(
      ONBOARDING_STORAGE_KEYS.TOUR_COMPLETED,
      String(hasCompletedTour),
    );
  }, [hasCompletedTour]);
  ...
  useEffect(() => {
    if (phase === "personalize") {
      trackOnboardingProgressMilestone(
    ...
    try {
      ...
      setProfileError(
        error instanceof Error
          ? error.message
          : "Could not save your player profile.",
      );
    } finally {
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-400 mt-1">
                See how SportWarren works before you start.
              </p>
            </div>
            <button
              onClick={startTour}
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[350] pointer-events-auto w-[320px]">
        ...
        >
          <div className="flex items-center space-x-3 mb-4">
            ...
            <button
              onClick={onSkip}
              className="p-2 text-gray-300 hover:text-gray-900 hover:bg-gray-200/50 rounded-lg"
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-100">
            ...
            <div className="flex space-x-1">
              {steps.map((_, i) => (
                <div
                  key={i}
                  className={`h-1 rounded-full transition-all ${i === currentStep ? "w-4 bg-gray-900" : "w-1 bg-gray-200"}`}
                />
              ))}
            </div>
...
function PersonalizationCard({
  ...
  const handleMatchResultClick = (
    result: (typeof SCORE_OPTIONS)[number]["id"],
  ) => {
  ...
  const MATCH_RESULT_XP: Record<string, number> = {
    won: 15,
    drew: 10,
    lost: 5,
  };
  const MATCH_RESULT_MULTIPLIER: Record<string, number> = {
    won: 1,
    drew: 0.6,
    lost: 0.3,
  };
  ...
  const displayAttributes: {
    ...
    return allKeys.map((key) => ({
      ...
      baseValue: base[key],
      boostedValue: Math.min(
        99,
        Math.max(1, Math.round(base[key] + (delta[key] ?? 0) * mult)),
      ),
    }));
  ...
  const addTeammate = () => {
    ...
    setTeammates((prev) => [
      ...prev,
      { name: newTeammateName.trim(), position: newTeammatePos },
    ]);
    setNewTeammateName("");
  ...
  const handleShareSquad = () => {
    trackEvent("onboarding_share_squad_clicked", {
      formation,
  ...
  const handleSharePlayerCard = () => {
    trackEvent("onboarding_share_player_card_clicked", {
      formation,
      matchResult,
    });
  ...
  return (
    <Card className="bg-gradient-to-br from-gray-900 to-black border-gray-800 text-white overflow-hidden relative shadow-2xl max-w-lg mx-auto">
      ...
      {(() => {
        const progress =
          step === "identity" ? 33 : step === "formation" ? 66 : 90;
        return (
          <div className="border-b border-white/10 px-8 py-3">
            ...
            {step === "brand" && (
              <p className="mt-1.5 text-[10px] font-bold text-amber-300">
                ⚡ You&apos;re one step away from launching your season — lock
                it in!
              </p>
            )}
          </div>
        ...
        <AnimatePresence mode="wait">
          {step === "identity" ? (
            ...
            >
              ...
              <p className="text-sm text-gray-400 mb-8">
                This defines how your squad lines up on match day.
              </p>
              ...
              {hasPendingPersona && (
                ...
                <div className="mb-6 rounded-2xl border border-emerald-500/30 bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 p-4 shadow-lg shadow-emerald-500/5">
                  ...
                  <p className="mt-3 text-[11px] leading-relaxed text-emerald-200/80">
                    These are your starting stats. They become real after your
              ...
              <div className="space-y-3 mb-6">
                {FORMATION_OPTIONS.map((option) => (
                  ...
                  >
                    <div className="bg-gray-800 rounded-xl p-3 text-xs font-black">
                      {option.id}
                    </div>
                    <div>
                      <div
                        className={`text-sm font-black uppercase ${formation === option.id ? "text-green-400" : "text-white"}`}
                      >
                        {option.label}
                      </div>
              ...
              {formation && !hasPendingPersona && (
                <div className="mb-6 rounded-2xl border border-white/10 bg-white/[0.04] p-5">
                  ...
                  {teammates.length > 0 && (
                    <p className="mt-3 text-[10px] text-gray-600 text-center">
                      You + {teammates.length} teammate
                      {teammates.length !== 1 ? "s" : ""} — add more or proceed
                    </p>
                  )}
                  {teammates.length === 0 && (
                  ...
                  {teammates.length > 0 &&
                    ...
                    FORMATIONS[formation as keyof typeof FORMATIONS] && (
                      <div className="mt-5">
                        ...
                        <div
                          className="relative mx-auto h-52 w-full max-w-[200px] overflow-hidden rounded-2xl"
                          style={{
                            background:
              ...
              <button
                ...
                className="w-full py-4 bg-white text-black rounded-2xl font-black uppercase tracking-[0.2em] text-sm hover:bg-green-400 transition-all disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {skipBrandStep ? "Launch Your Season" : "Define Your Identity"}{" "}
            ...
            >
              ...
              <p className="text-sm text-gray-400 mb-8">
                This identity will be shared with your teammates.
              </p>

              <div className="mb-6">
                <label
                  htmlFor="squad-nickname"
                  className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2 block"
                >
                  Squad Nickname
                </label>
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          ...
                          <p className="text-[10px] text-gray-500">
                            {formation} · {teammates.length + 1} players
                          </p>
                        </div>
                        <div className="flex -space-x-2">
                          ...
                          {teammates.slice(0, 3).map((t, i) => (
                            <div
                              key={i}
                              className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-white/30 bg-white/10 text-[8px] font-black text-gray-300"
                            >
                              {(t.name.slice(0, 2) || "SW").toUpperCase()}
                            </div>
                          ))}
...
function registerGoatAgent(
  ...
  fetch("/api/goat/register-agent", {
    ...
    console.debug(
      "GOAT agent registration skipped (not configured or unreachable)",
    );
  });
}
