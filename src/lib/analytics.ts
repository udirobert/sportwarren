...
const sanitizeProperties = (
  ...
  for (const key in sanitized) {
    ...
    if (typeof value === "string" && walletRegex.test(value)) {
      sanitized[key] =
        `${value.slice(0, 4)}...${value.slice(-4)} (PII_REDACTED)`;
    }
  }
...
export function trackEvent(
  ...
  if (typeof window !== "undefined") {
    window.dispatchEvent(
      new CustomEvent("sw-analytics-updated", { detail: { name: eventName } }),
    );
  }
  ...
  if (
    ...
    (window as unknown as SentryGlobal).Sentry
  ) {
 ...
 */
export function trackGuestMode(
  action: "started" | "migrated" | "prompted",
): void {
 ...
 */
export function trackMatchSubmission(
  matchId: string,
 ...
 */
export function trackCoreGrowthEvent(
  event: CoreGrowthEvent,
  properties?: EventProperties,
): void {
 ...
 */
export function getCoreGrowthRecords(
  events: AnalyticsEvent[] = getStoredEvents(),
): CoreGrowthRecord[] {
