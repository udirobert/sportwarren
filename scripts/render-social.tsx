/**
 * Generate 1080×1080 social-format PNGs for every archetype in
 * `CARDS_SOCIAL`. Used by the GTM workflow — captains and contributors
 * pull these directly for Instagram / X / LinkedIn posts without
 * reformatting.
 *
 *   pnpm tsx scripts/render-social.tsx
 *
 * Outputs:
 *   docs/makeathon/assets/social/<kind>-social.png
 *
 * No DB required — uses synthetic moment data shared with the
 * landscape `render-before-after.tsx` script.
 */
...
import {
  ...
  SOCIAL_WIDTH,
  SOCIAL_HEIGHT,
} from '../src/components/moments/cards/social';
import type { MomentForRender } from '../src/components/moments/cards/types';

const HERE = path.dirname(fileURLToPath(import.meta.url));
const ASSETS_DIR = path.resolve(HERE, '..', 'docs', 'makeathon', 'assets', 'social');
...
const SAMPLE_MOMENTS: Array<MomentForRender & { kind: string }> = [
  {
    ...
    detail: 'Marcus Tate — 28 goals — broke the squad record set 12 May 2019',
...
async function main() {
  ...
  for (const moment of SAMPLE_MOMENTS) {
    const Card = CARDS_SOCIAL[moment.kind];
    if (!Card) {
      console.log(`[${moment.kind}] no social variant yet — skipping`);
      skipped++;
      continue;
    }
    console.log(`[${moment.kind}] rendering 1080×1080…`);
    ...
    const outPath = path.join(ASSETS_DIR, `${moment.kind}-social.png`);
    await writeFile(outPath, png);
    console.log(`  → ${path.relative(process.cwd(), outPath)}  (${png.length} bytes)`);
    rendered++;
  }
  console.log(`\nDone. ${rendered} rendered, ${skipped} skipped (no social variant yet).`);
}
